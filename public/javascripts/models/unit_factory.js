...
Hexwar.UnitFactory.prototype.loadFromServer = function(url) {
	...
	var successFunction = function(data, textStatus, jqXHR) {
		...
		for (var n=0; n < data.length; n++) {
			...
			this.createUnitType({ name:obj.name, price:parseFloat(obj.price), img:obj.img, img_x:parseFloat(obj.img_x), img_y:parseFloat(obj.img_y)
					, attack_range:parseFloat(obj.attack_range), hard_attack_power:parseFloat(obj.hard_attack_power), soft_attack_power:parseFloat(obj.soft_attack_power)
					, defense_type:obj.defense_type, defense_power:parseFloat(obj.defense_power)
					, move_range:parseFloat(obj.move_range), move_costs:move_costs, defense_bonuses:defense_bonuses});
	 	}
	};
 ...
 */
Hexwar.UnitFactory.prototype.createUnitType = function(params) {
	var type = new Hexwar.UnitType(params);
